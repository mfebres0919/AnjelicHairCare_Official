...
/* =========================
   STRIPE MODAL (Payment Link)
   ========================= */

(function () {
  const modal = document.getElementById('stripeModal');
  const frame = document.getElementById('stripeModalFrame');
  const openLink = document.getElementById('stripeModalOpenLink');

  if (!modal || !frame || !openLink) return;

  const openModal = (url) => {
    // Set iframe + fallback link
    frame.src = url;
    openLink.href = url;
  ...
  const closeModal = () => {
    ...
    // Stop iframe content (prevents audio/CPU + resets)
    frame.src = '';
    openLink.href = '#';
  };

  // Intercept "BUY NOW" buttons
  document.addEventListener('click', (e) => {
    const btn = e.target.closest('.js-stripe-modal');
    if (!btn) return;
  ...
  // Close handlers (overlay + close buttons)
  modal.addEventListener('click', (e) => {
    if (e.target.matches('[data-stripe-close]')) closeModal();
  });
